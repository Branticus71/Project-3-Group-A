...
`out_params` is a vector of parameters which are used as input parameters to **Project-3-Group-A.Rmd** file. Since the `param` argument in `render()` function requires params to be a named list, we have converted each element of `out_params` to a named list using `lapply()` into `out_params_list`.  
...
out_filename <- c("Lifestyle_Channel.md","Entertainment_Channel.md",
                  "Business_Channel.md","Social_media_Channel.md",
                  "Tech_Channel.md","World_Channel.md")
for (i in 1:6){
  rmarkdown::render(input = "Project-3-Group-A.Rmd", output_file = out_filename[i],
